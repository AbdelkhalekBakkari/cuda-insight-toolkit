...
__device__ void PopulateSharedMem2D(T *sharedMem, texture<T, 2> texRef,
		int2 imageDim, int2 radius, int2 pixel, int2 kernelDim) {
	for (int j = 0; j < kernelDim.y; j += blockDim.y) {
		for (int i = 0; i < kernelDim.x; i += blockDim.x) {
			if ((threadIdx.x + i) < kernelDim.x && (threadIdx.y + j) < kernelDim.y) {
				if ((pixel.y - radius.y + j) > -1 && (pixel.y - radius.y + j)
						< imageDim.y && (pixel.x - radius.x + i) > -1
						&& (pixel.x - radius.x + i) < imageDim.x) {
					sharedMem[threadIdx.x + i + ((threadIdx.y + j) * kernelDim.x)]
...
__device__ void PopulateSharedMem3D(T *sharedMem, texture<T, 3> texRef,
		int3 imageDim, int3 radius, int3 pixel, int3 kernelDim) {
	for (int k = 0; k < kernelDim.z; k += blockDim.z) {
		for (int j = 0; j < kernelDim.y; j += blockDim.y) {
			for (int i = 0; i < kernelDim.x; i += blockDim.x) {
				if ((threadIdx.x + i) < kernelDim.x && (threadIdx.y + j)
						< kernelDim.y && (threadIdx.z + k) < kernelDim.z) {
					if ((pixel.z - radius.z + k) > -1 && (pixel.z - radius.z
						...
						sharedMem[threadIdx.x + i + ((threadIdx.y + j)
								* kernelDim.x) + ((threadIdx.z + k) * kernelDim.y
								* kernelDim.x)] = tex3D(texRef, pixel.x - radius.x
...
__device__ T GetSharedMemValue2D(T* sharedMem, int i, int j, int2 imageDim,
		int2 radius, int2 pixel, int2 kernelDim) {
	...
	return sharedMem[x + (y * kernelDim.x)];
}
...
__device__ T GetSharedMemValue3D(T* sharedMem, int i, int j, int k,
	...
	return sharedMem[x + ((y + (z * kernelDim.y)) * kernelDim.x)];
}
